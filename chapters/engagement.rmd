...
We report the top communities most engaged in commercial and recreational fisheries and the degree to which these communities may be vulnerable to environmental justice issues (i.e., Poverty, Population Composition, and Personal Disruption). To select and present these communities we developed indicators (or indices) that inform the importance of fishing and relative social conditions in each community.
...
The commercial fishing engagement index measures the number of permits and dealers, and pounds and value of fish landed in a community. The relative engagement index is a proxy for how engaged each community is in fishing relative to its total population size. For this report, we selected the top ten communities with the highest engagement scores and the top ten communities with the highest relative engagement scores. Sometimes, a community will appear on both lists (i.e., top ten highest engagement AND relative engagement scores) such that the total report top communities will sum to less than 20. Recreational fishing engagement measures shore, private vessel, and for-hire fishing effort while relative engagement expresses these numbers based on fishing effort relative to the population of a community.

In 2024, we reported the top ten most engaged commercial and recreational fishing communities and their associated environmental justice vulnerability based on 2021 data. We had also reported communities with the highest relative engagement scores, but previously they were named reliance indicators. Here, we recast commercial and recreational reliance indicators as relative engagement indicators given that they are a proxy for how engaged each community is in fishing relative to its total population size and many more factors ultimately contribute to a fishing community's reliance on fishing. Importantly, the calculation of these indicators remains the same.

Here we apply the same selection standard for top fishing communities for both sectors using 2022 data, and again examine the environmental justice vulnerability in this updated set of communities. Changes in fishing activity between years relative to other communities changed community engagement and relative engagement rankings, and changes in vulnerability indicators relative to other communities changed environmental justice vulnerability scores.

## Key Results and Visualizations
**** ALL OF THE BELOW STILL NEEDS TO BE UPDATED WITH NEW RESULTS ****
...
```{r plot_engagementMidAtlanticCommercialEJ}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'MidAtlantic', varName= 'Commercial' ,plottype= 'EJ')
ggplotObject
```

```{r plot_engagementMidAtlanticCommercialEconomic}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'MidAtlantic', varName= 'Commercial' ,plottype= 'Economic')
ggplotObject
```

```{r plot_engagementMidAtlanticCommercialGentrification}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'MidAtlantic', varName= 'Commercial' ,plottype= 'Gentrification')
ggplotObject
```

```{r plot_engagementMidAtlanticRecreationalEJ}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'MidAtlantic', varName= 'Recreational' ,plottype= 'EJ')
ggplotObject
```

```{r plot_engagementMidAtlanticRecreationalEconomic}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'MidAtlantic', varName= 'Recreational' ,plottype= 'Economic')
ggplotObject
```

```{r plot_engagementMidAtlanticRecreationalGentrification}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'MidAtlantic', varName= 'Recreational' ,plottype= 'Gentrification')
ggplotObject
```

### NewEngland

```{r plot_engagementNewEnglandCommercialEJ}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'NewEngland', varName= 'Commercial' ,plottype= 'EJ')
ggplotObject
```

```{r plot_engagementNewEnglandCommercialEconomic}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'NewEngland', varName= 'Commercial' ,plottype= 'Economic')
ggplotObject
```

```{r plot_engagementNewEnglandCommercialGentrification}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'NewEngland', varName= 'Commercial' ,plottype= 'Gentrification')
ggplotObject
```

```{r plot_engagementNewEnglandRecreationalEJ}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'NewEngland', varName= 'Recreational' ,plottype= 'EJ')
ggplotObject
```

```{r plot_engagementNewEnglandRecreationalEconomic}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'NewEngland', varName= 'Recreational' ,plottype= 'Economic')
ggplotObject
```

```{r plot_engagementNewEnglandRecreationalGentrification}
# Plot indicator
ggplotObject <- ecodata::plot_engagement(report= 'NewEngland', varName= 'Recreational' ,plottype= 'Gentrification')
ggplotObject
```
...
**Point of contact**: [Bobby Murphy (robert.murphy@noaa.gov)](mailto:Bobby Murphy (robert.murphy@noaa.gov)){.email}
...
10) 1 std: 1 standard deviation. 11) 0.5 std: 0.5 standard deviation.
